document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- Konstanter ---
    const GAS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbzaG6_GQeWYBKJ4etlTH1UWavFSPcU95aUXTcL9hEXYksWah7FLNpLYOemloMLZ4xMt/exec'; // ERSTATT DENNE
    ...
    const newsletterNameInput = document.getElementById('newsletter-name'); // NYTT: Referanse til navnefelt
    ...
    function renderOptions() { const currentVisibleStepNum = calculateCurrentVisibleStep(); if (currentStep > steps.length) { if (!showRecommendationsView) { generateAndShowRecommendations(); } return; } const stepDef = getStepDefinition(currentVisibleStepNum); if (!stepDef) { if (!showRecommendationsView) { generateAndShowRecommendations(); } return; } if (stepTitle) stepTitle.textContent = stepDef.title; if (stepOptions) { stepOptions.innerHTML = ''; stepDef.options.forEach((option) => { const button = document.createElement('button'); button.classList.add('option-button'); if (option.className) { button.classList.add(option.className); } button.dataset.value = option.id; if (option.className === 'cargo-type') { button.innerHTML = `${option.image ? `<img src="${option.image}" alt="${option.label}" onerror="this.style.display='none'; this.nextSibling.style.marginTop='0';">` : ''}<div><div>${option.label}</div>${option.description ? `<div class="description">${option.description}</div>` : ''}</div>`; } else { button.innerHTML = `<div>${option.label}</div>${option.description ? `<div class="description">${option.description}</div>` : ''}`; } if (selections[stepDef.id] === option.id) { button.classList.add('selected'); } button.addEventListener('click', () => handleOptionSelect(stepDef.id, option.id)); stepOptions.appendChild(button); }); } updateProgress(); }
    ...
    // --- Oppdater Visning ---
    function updateView() { totalSteps = calculateTotalVisibleSteps(); if (showRecommendationsView) { if (questionsSection) questionsSection.classList.add('hidden'); if (recommendationsSection) recommendationsSection.classList.remove('hidden'); if (contactEvoSection) { recommendations.length > 0 ? contactEvoSection.classList.remove('hidden') : contactEvoSection.classList.add('hidden'); } if (newsletterSection) { recommendations.length > 0 ? newsletterSection.classList.remove('hidden') : newsletterSection.classList.add('hidden'); } } else { if (BikeCatalog.evoOriginal && BikeCatalog.evoOriginal.length > 0) { if (questionsSection) questionsSection.classList.remove('hidden'); } else { if (initialLoader && !initialLoader.classList.contains('hidden')) { if (questionsSection) questionsSection.classList.add('hidden'); } else if (BikeCatalog.evoOriginal.length === 0 && initialLoader && initialLoader.classList.contains('hidden')) { if (questionsSection) questionsSection.classList.add('hidden'); } } if (recommendationsSection) recommendationsSection.classList.add('hidden'); if (contactEvoSection) contactEvoSection.classList.add('hidden'); if (newsletterSection) newsletterSection.classList.add('hidden'); if (loadingIndicator) loadingIndicator.classList.add('hidden'); if (recommendationsOutput) recommendationsOutput.classList.add('hidden'); renderSentence(sentenceBuilder); renderOptions(); const currentVisible = calculateCurrentVisibleStep(); if (backButton) backButton.classList.toggle('hidden', currentVisible <= 1 && currentStep <= 1); } updateProgress(); }

    // --- Generer Anbefalinger ---
    function generateAndShowRecommendations() { relaxedSearchPerformed = false; showRecommendationsView = true; if (questionsSection) questionsSection.classList.add('hidden'); if (recommendationsSection) { recommendationsSection.classList.remove('hidden'); recommendationsSection.scrollIntoView({ behavior: 'smooth', block: 'start' }); } if (recommendationsOutput) recommendationsOutput.classList.add('hidden'); if (contactEvoSection) contactEvoSection.classList.add('hidden'); if (newsletterSection) newsletterSection.classList.add('hidden'); if (loadingIndicator) loadingIndicator.classList.remove('hidden'); renderSentence(summarySentenceFinal); setTimeout(() => { const filterBikes = (relaxFrameType = false, relaxDistance = false, purposeOnly = false) => { let bikesToFilter = [...BikeCatalog.evoOriginal]; if (selections.purpose && !purposeOnly) { bikesToFilter = bikesToFilter.filter(bike => bike.purpose && Array.isArray(bike.purpose) && bike.purpose.includes(selections.purpose)); } else if (purposeOnly && selections.purpose) { return bikesToFilter.filter(bike => bike.purpose && Array.isArray(bike.purpose) && bike.purpose.includes(selections.purpose)); } else if (purposeOnly && !selections.purpose) { return bikesToFilter; } if (!purposeOnly) { if (selections.distance && !relaxDistance) { let minRange = 0; if (selections.distance === 'medium') minRange = 20; else if (selections.distance === 'lang') minRange = 50; bikesToFilter = bikesToFilter.filter(bike => bike.distance_km && Array.isArray(bike.distance_km) && bike.distance_km[1] >= minRange); } if (selections.cargo) { let cargoLevels = []; if (selections.cargo === 'små') cargoLevels = ['small', 'medium', 'large', 'massive']; else if (selections.cargo === 'store') cargoLevels = ['medium', 'large', 'massive']; else if (selections.cargo === 'massiv') cargoLevels = ['large', 'massive']; bikesToFilter = bikesToFilter.filter(bike => bike.cargo_capacity && cargoLevels.includes(bike.cargo_capacity)); } if (selections.frameType && !relaxFrameType) { let frameTypesToMatch = []; if (selections.frameType === 'dypGjennomgang') frameTypesToMatch = ['low-step', 'cargo', 'cargo-longtail']; else if (selections.frameType === 'lavtTopprør') frameTypesToMatch = ['mid-step']; else if (selections.frameType === 'høytTopprør') frameTypesToMatch = ['high-step']; bikesToFilter = bikesToFilter.filter(bike => bike.frame_types && Array.isArray(bike.frame_types) && bike.frame_types.some(type => frameTypesToMatch.includes(type))); } const cargoLocationStep = steps.find(s => s.id === 'cargoLocation'); const cargoLocationAsked = !cargoLocationStep || !cargoLocationStep.condition || cargoLocationStep.condition(); if (cargoLocationAsked && selections.purpose === 'transport' && selections.cargoLocation) { const locationToMatch = selections.cargoLocation === 'frontlaster' ? 'front' : 'rear'; bikesToFilter = bikesToFilter.filter(bike => bike.cargo_location === locationToMatch); } } return bikesToFilter; }; let potentialMatches = filterBikes(); if (potentialMatches.length === 0) { potentialMatches = filterBikes(true, false); if (potentialMatches.length > 0) relaxedSearchPerformed = true; } if (potentialMatches.length === 0) { potentialMatches = filterBikes(false, false, true); if (potentialMatches.length > 0) relaxedSearchPerformed = true; } potentialMatches.sort((a, b) => { const inferChildNeed = selections.purpose === 'transport' || selections.cargo === 'massiv' || selections.cargo === 'store'; const a_meets_child = (a.maxChildren && a.maxChildren > 0); const b_meets_child = (b.maxChildren && b.maxChildren > 0); if (inferChildNeed) { if (a_meets_child && !b_meets_child) return -1; if (!a_meets_child && b_meets_child) return 1; if (a_meets_child && b_meets_child) { return (b.maxChildren || 0) - (a.maxChildren || 0); } } return (parseFloat(String(a.price).replace(/[^0-9.]/g, '')) || Infinity) - (parseFloat(String(b.price).replace(/[^0-9.]/g, '')) || Infinity); }); recommendations = potentialMatches.slice(0, 3); if (loadingIndicator) loadingIndicator.classList.add('hidden'); renderRecommendations(); if (recommendationsOutput) recommendationsOutput.classList.remove('hidden'); if (contactEvoSection) { recommendations.length > 0 ? contactEvoSection.classList.remove('hidden') : contactEvoSection.classList.add('hidden'); } if (newsletterSection) { recommendations.length > 0 ? newsletterSection.classList.remove('hidden') : newsletterSection.classList.add('hidden'); } }, 300); }
    ...
    async function subscribeToMailchimpViaGAS(navn, email, recommendedBikes = []) { // Lagt til 'navn' parameter
        if (!GAS_WEB_APP_URL || GAS_WEB_APP_URL === 'DIN_PUBLISERTE_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HER') {
    ...
    if (newsletterForm) {
        newsletterForm.addEventListener("submit", async (e) => {
            e.preventDefault();
            if (!newsletterNameInput || !newsletterEmailInput || !newsletterConsentCheckbox || !newsletterMessage) return; // Sjekker også newsletterNameInput
            
            const navn = newsletterNameInput.value.trim(); // Henter navn
            const email = newsletterEmailInput.value.trim();

            if (!navn) { // Validerer at navn er fylt ut
            ...
            try {
                await subscribeToMailchimpViaGAS(navn, email, recommendations); // Sender med 'navn'
                newsletterMessage.textContent = "Takk! Sykkelforslaget er på vei til din e-post.";
                newsletterMessage.style.color = "#198754";
                newsletterNameInput.value = ""; // Tømmer navnefeltet
