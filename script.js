document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Steps definisjon (uten childCapacity) ---
    const steps = [
        { id: 'purpose', title: 'Jeg trenger en sykkel for', options: [ { id: 'pendling', label: 'Turer eller pendling til jobb' }, { id: 'bybruk', label: 'Være ute i byen / Koseturer' }, { id: 'terreng', label: 'Bevege meg utenfor veien (sti/grus)' }, { id: 'transport', label: 'Transportere mye (varer/barn)' }, { id: 'allsidig', label: 'En allsidig sykkel til "litt av alt"' } ] },
    ...
    let totalSteps = calculateTotalVisibleSteps(); // Initial calculation
    ...
    // --- Rendering Funksjoner (renderSentence oppdatert, renderRecommendations inkluderer hint) ---
     ...
     let relaxedSearchPerformed = false; // Flyttet utenfor for å være tilgjengelig i renderRecommendations
     function renderRecommendations() {
         ...
         if (relaxedSearchPerformed) {
            ...
            notice.textContent = "Ingen sykler passet 100% til alle dine valg, så her er de nærmeste alternativene:";
            notice.style.cssText = 'font-size: 0.9em; font-style: italic; margin-bottom: 15px; color: #6c757d;';
            recommendationsOutput.appendChild(notice);
         }
         recommendations.forEach((bike, index) => {
             const card = document.createElement('div'); card.classList.add('recommendation-card');
             let badgeText = ''; if(bike.preOrdered && index === 0) badgeText = 'TOPPVALG'; else if (bike.preOrdered) badgeText = 'ANBEFALT'; else if (index === 0) badgeText = 'TOPPVALG'; else if (index === 1) badgeText = 'GOD MATCH'; else badgeText = 'ALTERNATIV';
    ...
    function generateAndShowRecommendations() {
        ...
        relaxedSearchPerformed = false; // Nullstill flagget
        ...
        setTimeout(() => {
            ...
            if (potentialMatches.length === 0) {
                ...
                potentialMatches = filterBikes(true);
             ...
             if (potentialMatches.length === 0 && selections.purpose) {
                 ...
                 potentialMatches = filterBikes(false, false, true);
                 relaxedSearchPerformed = true;
            ...
            // Prioritert Sortering
             potentialMatches.sort((a, b) => {
                 ...
                 const a_children = a.maxChildren ?? -1; const b_children = b.maxChildren ?? -1; if (a_children !== b_children && inferChildNeed) { return b_children - a_children; }
